...
class FunctionalImagingModule(LightningModule):
    def __init__(self):
        ...
        self.train_codes = torch.randn((config.image.cells * 2) ** 2, config.latent.dim) * 0.001
    ...
    def linear_blending(self, im, imsize, cell_width):
        # cell_width = self.train_dataset.cell_width 
        idcs = np.arange(cell_width // 2, 2 * imsize -
    ...
    def log_images(self):
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                ground_truth = batch['large_img'].cuda()
                # batch['coords'] = batch['coords'].cuda()
                batch['coords'] = batch['large_coords'].cuda()
                ...
                img_out = img_out.view(
                    # config.training.batch_size, config.image.channels, self.train_dataset.cell_width, self.train_dataset.cell_width)
                    config.training.batch_size, config.image.channels, self.train_dataset.cell_width * 2, self.train_dataset.cell_width * 2)
        ...
        # if config.model.local:
            # for i in range(config.image.channels):
                # out_i = unblockshaped_tensor_single(
                        # out_im[:, i], 4*config.image.size, 4*config.image.size)
                # gt_i = unblockshaped_tensor_single(
                        # gt_im[:, i], 4*config.image.size, 4*config.image.size)

                # out_i = self.linear_blending(out_i, config.image.size * 2, self.train_dataset.cell_width * 2)
                # out_i = self.linear_blending(out_i.permute(1, 0), config.image.size * 2, self.train_dataset.cell_width * 2).permute(1, 0)
                # gt_i = self.linear_blending(gt_i, config.image.size * 2, self.train_dataset.cell_width * 2)
                # gt_i = self.linear_blending(gt_i.permute(1, 0), config.image.size * 2, self.train_dataset.cell_width * 2).permute(1, 0)
        ...
        for i in range(config.image.channels):
            out_i = unblockshaped_tensor_single(
                    out_im[:, i], config.image.size * 2, config.image.size * 2)
            gt_i = unblockshaped_tensor_single(
                    gt_im[:, i], config.image.size * 2, config.image.size * 2)
...
if __name__ == "__main__":
    ...
    config['image']['cells'] = config.image.size // config.image.cell_width
