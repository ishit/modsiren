...
from modsiren.datasets_bak import DynamicDragon, DragonDataset
...
class FunctionalImagingModule(LightningModule):
    def __init__(self):
        ...
        self.train_dataset = DragonDataset(n_voxels=config.shape.n_voxels,\
        ...
        # self.train_dataset = DynamicDragon(n_voxels=config.shape.n_voxels,\
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step % 1000 == 0 and self.global_step > 1:
            cd = self.render()
    ...
    def train_dataloader(self):
        dataloader = DataLoader(
            self.train_dataset, batch_size=config.training.batch_size, shuffle=False, pin_memory=True, num_workers=0, drop_last=True)
        return dataloader
