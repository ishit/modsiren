...
echo "model" $model_id "video" $VID_ID
...
# get config scripts
CONFIG_DIR=$CODE_LOCAL/config/single_vid/
...
VID_PATH=bikes_processed/$VID_ID.npy

# get data
echo "downloading data"
mkdir -p bikes_processed
rsync -av --ignore-existing $DATA/bikes_processed/$VID_ID.npy $VID_PATH
# rsync -av --ignore-existing $DATA/bikes_processed.zip .

# echo extracting data
# if [ ! -d "bikes_processed" ] 
# then
#     unzip bikes_processed.zip
# fi
...
LOG_DIR=$OUTPUT/logs_single_vid
...
sed -i "/^exp_name:/ s/$/_$VID_ID/" temp_config.yml

python -m scripts.train_single_vid --config temp_config.yml
echo "job done"
